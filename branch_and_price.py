...
class Branch_and_Bound():
    ...
    def generate_upper_bound(self, lb_sol, time_matrix, num_customers):
        ...
        for entry in fractional_routes:
            if len(customers_covered) < num_customers:
                route = entry[1]
                route_adj = [route[x] for x in range(len(route))]
    ...
    def branch(self, vehicle_capacity, time_matrix, demands,
        ...
        if lb_obj1 < self.best_ub and ub_sol1 != []:
            ...
            edge1 = self.determine_branching_rule(fractional_routes1)
        ...
        if lb_obj2 < self.best_ub and ub_sol2 != []:
            ...
            edge2 = self.determine_branching_rule(fractional_routes2)
